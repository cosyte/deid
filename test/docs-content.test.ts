 ...
 *
 * `resolve` points a snippet's `import ... from "@cosyte/deid"` at this repo's source, the same way the
...
docSnippetSuite({
  ...
  resolve: (specifier) =>
    specifier === "@cosyte/deid" ? join(import.meta.dirname, "..", "src", "index.ts") : undefined,
});
