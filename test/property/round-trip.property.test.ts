 ...
 * **format-specific arbitraries** (the `Deid` generators below).
 *
 * This file is the intended shape for every `@cosyte/*` parser (see `@cosyte/hl7`'s
 * `test/property/`). While `@cosyte/deid` is still a scaffold:
 *
 ...
 *   - the **round-trip** invariant is `it.todo` until the serializer (`serializeDeid` /
...
import { FATAL_CODES, WARNING_CODES, parseDeid, type ParsedDeid } from "../../src/index.js";
 ...
 * just arbitrary strings; replace it with a generator that emits real De-identification quirks (truncated
 ...
 */
function specCleanDeid(): fc.Arbitrary<ParsedDeid> {
  return fc.constant({ value: {}, warnings: [] } satisfies ParsedDeid);
}

describe("deid conformance (archetype invariants)", () => {
  it("is lenient — arbitrary input never throws a non-fatal, and every warning has a known code", () => {
    lenientNeverThrowsProperty({
      ...
      parse: (raw: string) => parseDeid(raw),
      ...
      getWarnings: (parsed) => (parsed as ParsedDeid).warnings,
  ...
  // TODO: flip `it.todo` -> `it` once a serializer (`serializeDeid` / `result.toString()`)
