...
Common symptoms when integrating `@cosyte/deid`, and how to read what the parser is telling you.

## The parse "succeeded" but the result looks wrong

`@cosyte/deid` is lenient — it recovers from vendor quirks rather than throwing. That means a surprising
...
> **Status:** `@cosyte/deid` is a pre-alpha scaffold. `parseDeid` currently returns a structural stub
